...
\begin{itemize}
\item Aggregate the diamonds data by cut and color; calculate the mean and standard deviation of price.\\
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{d3} \hlkwb{<-} \hlkwd{subset}\hlstd{(diamonds, cut}\hlopt{%in%}\hlkwd{c}\hlstd{(}\hlstr{"Fair"}\hlstd{,} \hlstr{"Ideal"}\hlstd{))}
\hlstd{d4} \hlkwb{<-} \hlkwd{subset}\hlstd{(d3, color}\hlopt{%in%}\hlkwd{c}\hlstd{(}\hlstr{"E"}\hlstd{,} \hlstr{"J"}\hlstd{))}
\hlstd{dagg} \hlkwb{<-} \hlkwd{ddply}\hlstd{(d4,} \hlkwd{.}\hlstd{(cut, color), summarise,}
              \hlkwc{mean_price} \hlstd{=} \hlkwd{mean}\hlstd{(price),}
              \hlkwc{sd_price} \hlstd{=} \hlkwd{sd}\hlstd{(price))}
\hlstd{dodge} \hlkwb{<-} \hlkwd{position_dodge}\hlstd{(}\hlkwc{width}\hlstd{=}\hlnum{0.5}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\centering \includegraphics[width=.75\linewidth]{figure/ex5} 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: could not find function "{}display.wes.palette"{}}}\end{kframe}
\end{knitrout}
\end{center}
\end{frame}
