...
\begin{itemize}
\item Use \texttt{plyr} to calculate the mean and standard deviation of petal length by species.\\
\begin{knitrout}\footnotesize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{agg} \hlkwb{<-} \hlkwd{ddply}\hlstd{(iris,} \hlkwd{.}\hlstd{(Species), summarise,}
             \hlkwc{mean.petal.length} \hlstd{=} \hlkwd{mean}\hlstd{(Petal.Length),}
             \hlkwc{sd.petal.length} \hlstd{=} \hlkwd{sd}\hlstd{(Petal.Length))}
\hlopt{?}\hlkwd{geom_errorbar}\hlstd{()}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\centering \includegraphics[width=.75\linewidth]{figure/facet_wrap_theme_exec_royal} 
